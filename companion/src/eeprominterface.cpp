...
bool RawSource::isTimeBased(bool timerOnly) const
{
  if (IS_ARM(getCurrentBoard()))
    return (type == SOURCE_TYPE_SPECIAL && index > (int)timerOnly);
  else
    return (type == SOURCE_TYPE_TELEMETRY && ((index == TELEMETRY_SOURCE_TX_TIME && !timerOnly) || (index >= TELEMETRY_SOURCE_TIMER1 && index <= TELEMETRY_SOURCE_TIMER3)));
}
